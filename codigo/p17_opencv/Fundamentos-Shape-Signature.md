...
Realice las siguientes actividades en base al ejemplo dado:

* Calcular y dibujar la firma de forma (Shape Signature) del polígono rotado, trasladado y escalado.
* Calcular la DFT de la firma de los dos polígonos (previamente se debe usar interpolación lineal para tener el mismo número de puntos en ambas firmas).
* Normalizar el resultado obtenido de la DFT de los polígonos.
* Comparar y visualizar los 12 primeros valores obtenidos.

## Cómo calcular la FFT para la práctica 4.1, 4.2

Existe una técnica mucho más poderosa, ya que en lugar de usar la distancia real $r(n)$, empleamos el plano complejo:

$$
    z(n) = (x_{n} - cx) + j(y_{n} - cy)
$$

Si introducimos este vector completjo directamente en la FFT (*dft*) de OpenCV (usando una matriz de dos canales), obtendremos un descriptor que no solo es invariante, sino que también permite **reconstruir la figura original** inviertiendo el proceso (la inversa de la FFT) algo que no podemos hacer si solo guardamos las distancias del centroide a los puntos del contorno. **Esto es lo que se usa en sistemas de reconocimiento de escritura de alta gama**.
